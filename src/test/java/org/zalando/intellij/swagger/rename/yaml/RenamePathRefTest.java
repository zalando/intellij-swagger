...
public class RenamePathRefTest extends SwaggerLightCodeInsightFixtureTestCase {
  ...
  public void testRenameExternalPathDefinitionWhereDefinitionsAreInRoot() {
    ...
    final VirtualFile swaggerFile =
        myFixture.copyFileToProject(
            FILES_PATH + "external_definition_path_definitions_in_root_with_caret.yaml",
            "swagger.yaml");
    myFixture.configureFromExistingVirtualFile(swaggerFile);
    myFixture.renameElementAtCaret("newName");
    myFixture.checkResultByFile(
        "swagger.yaml",
        FILES_PATH + "external_definition_path_definitions_in_root_with_caret_after.yaml",
        true);
    myFixture.checkResultByFile(
        "paths.yaml", FILES_PATH + "path_definitions_in_root_after.yaml", true);
  }
  ...
  public void testRenameExternalPathDefinitionWhereDefinitionsAreNotInRoot() {
    ...
    final VirtualFile swaggerFile =
        myFixture.copyFileToProject(
            FILES_PATH + "external_definition_path_definitions_not_in_root_with_caret.yaml",
            "swagger.yaml");
    myFixture.configureFromExistingVirtualFile(swaggerFile);
    myFixture.renameElementAtCaret("newName");
    myFixture.checkResultByFile(
        "swagger.yaml",
        FILES_PATH + "external_definition_path_definitions_not_in_root_with_caret_after.yaml",
        true);
    myFixture.checkResultByFile(
        "paths.yaml", FILES_PATH + "path_definitions_not_in_root_after.yaml", true);
  }
}
