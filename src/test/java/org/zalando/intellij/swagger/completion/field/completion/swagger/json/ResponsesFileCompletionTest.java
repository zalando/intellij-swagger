...
public class ResponsesFileCompletionTest extends SwaggerLightCodeInsightFixtureTestCase {
  ...
  public void testThatAutoCompletionWorksForFileWithSingleDefinition() {
    ...
    final AssertableList completions =
        new AssertableList(myFixture.getCompletionVariants("response.json"));
  ...
  public void testThatAutoCompletionWorksForFileWithMultipleResponseDefinitionsInRoot() {
    myFixture.copyFileToProject(
        PARTIAL_FILES_PATH + "/response_definitions_in_root.json",
        "response_definitions_in_root.json");
    final VirtualFile swaggerFile =
        myFixture.copyFileToProject(PARTIAL_FILES_PATH + "/swagger.json", "swagger.json");
    myFixture.configureFromExistingVirtualFile(swaggerFile);

    final AssertableList completions =
        new AssertableList(myFixture.getCompletionVariants("response_definitions_in_root.json"));
  ...
  public void testThatAutoCompletionWorksForFileWithMultipleResponseDefinitionsNotInRoot() {
    myFixture.copyFileToProject(
        PARTIAL_FILES_PATH + "/response_definitions_not_in_root.json",
        "response_definitions_not_in_root.json");
    final VirtualFile swaggerFile =
        myFixture.copyFileToProject(PARTIAL_FILES_PATH + "/swagger.json", "swagger.json");
    myFixture.configureFromExistingVirtualFile(swaggerFile);
    ...
    final AssertableList completions =
        new AssertableList(
            myFixture.getCompletionVariants("response_definitions_not_in_root.json"));
