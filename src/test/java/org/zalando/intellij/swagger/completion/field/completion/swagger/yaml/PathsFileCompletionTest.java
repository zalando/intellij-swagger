...
public class PathsFileCompletionTest extends SwaggerLightCodeInsightFixtureTestCase {
    ...
    public void testThatAutoCompletionWorksForFileWithSingleDefinitionForYaml() {
        ...
        final AssertableList completions = new AssertableList(myFixture.getCompletionVariants("path.yaml"));
    ...
    public void testThatAutoCompletionWorksForFileWithMultiplepathDefinitionsInRootForYaml() {
        myFixture.copyFileToProject(PARTIAL_FILES_PATH + "/path_definitions_in_root.yaml", "path_definitions_in_root.yaml");
        final VirtualFile swaggerFile = myFixture.copyFileToProject(PARTIAL_FILES_PATH + "/swagger.yaml", "swagger.yaml");
        myFixture.configureFromExistingVirtualFile(swaggerFile);

        final AssertableList completions = new AssertableList(myFixture.getCompletionVariants("path_definitions_in_root.yaml"));
    ...
    public void testThatAutoCompletionWorksForFileWithMultiplepathDefinitionsNotInRootForYaml() {
        myFixture.copyFileToProject(PARTIAL_FILES_PATH + "/path_definitions_not_in_root.yaml", "path_definitions_not_in_root.yaml");
        final VirtualFile swaggerFile = myFixture.copyFileToProject(PARTIAL_FILES_PATH + "/swagger.yaml", "swagger.yaml");
        myFixture.configureFromExistingVirtualFile(swaggerFile);

        final AssertableList completions = new AssertableList(myFixture.getCompletionVariants("path_definitions_not_in_root.yaml"));
